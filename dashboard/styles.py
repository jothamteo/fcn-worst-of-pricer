...
def render_what_is_an_fcn() -> None:
    ...
    with st.expander("What is an FCN?", expanded=False):
        st.markdown(
            ...
            "**Frame convention used in this dashboard.** The P&L, "
            "Attribution waterfall, and Greeks panels are shown from the "
            "**investor's** perspective — the natural side for reading "
            "what's happening to the structure's value: positive Δ (long "
            "basket), negative vega (short vol), positive cega (long "
            "correlation). These match notebooks 03-08. The **Hedging** "
            "tab flips signs to the dealer's side because that's where "
            "operational language lives — what shares the desk buys, what "
            "options it sells. The two are mirror images of the same risk."
        )
